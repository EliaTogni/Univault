...
Infine, la funzione di output è la funzione identità. E' possibile, quindi, rappresentare un HN attraverso la sua matrice dei pesi, di dimensione $n \times n$:
...
se le attivazioni di un HN sono aggiornate in modo asincrono, allora uno stato stabile viene raggiunto in un numero finito di passi. Se i neuroni sono attraversati in un fashion arbitrario ma ciclico, sono necessari al più $n\cdot 2^n$ passi, dove $n$ è il numero dei neuroni della rete.
...
La funzione di eenergia di un Hopfield Network con $n$ neuroni $u_1, ..., u_n$ è definita come:
