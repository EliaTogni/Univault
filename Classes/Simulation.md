...
### Gaussian copula
The two random variables $X$ and $Y$ that have known correlation $\rho$.  We generate a pair of values $Z = (z_1, z_2)$ from a bivariate Normal with the same correlation $\rho$. Then we obtain pairs of random uniform values correlated among them by computing $U= (u_1,u_2) = (\Phi(z_1,z_2))$. This two uniform random variable are still correlated and are then used to obtain a random couple of observation of the original random variables using the inverse of their CDF: $R=(F^{-1}(u_1), G^{-1}(u_2))$. The couple generated has the same correlation value of the variables $X$ and $Y$.
