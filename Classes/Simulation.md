...
- **Marshall-Olkin copula** $\to$ good to model chain relation.

### Gaussian copula
The two random variables $X$ and $Y$ that have known correlation $\rho$.  We generate a pair of values $Z = (z_1, z_2)$ from a bivariate Normal with the same correlation $\rho$. Then we obtain pairs of random uniform values correlated among them by computing $U= (u_1,u_2) = (\Phi(z_1,z_2))$. This two uniform random variable are still correlated and are then used to obtain a random couple of observation of the original random variables using the inverse of their Cumulative Distribution Function:
...
for the same reasoning $G_t(Y) = G(t^{-1}(y))$. However, $F_s(s(x)) = F(s^{-1}(s(x))) = F(x)$ and $G_t(t(x)) = G(t^{-1}(t(y))) = G(y)$, so:
