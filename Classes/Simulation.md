...
Our understanding of the normal is telling us, indeed, that if we take a normal random variable, we multiply it by a constant and add/subtract another constant, we still get a normal random variable. However, this is still tricky because we don't know $\sigma$: hence, we replace it with $S$, an estimate of it.
