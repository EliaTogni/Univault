...
K(x,x′) = ϕ(x)⊤ϕ(x′) for all x,x′ ∈Rd. (1)

For example, the quadratic kernel, corresponding to the quadratic feature-expansion map

√ √ √ ![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.002.png)![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.003.png)![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.004.png)

2  2 

ϕ(x1,x2) = 1,x1,x2, 2x1, 2x2, 2x1x2

√ ![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.005.png)

is K(x,x′) = 1 + x⊤x′ 2, as one can easily verify (the presence of the 2 coefficients, which is needed for the math, does not change the class of linear predictors that are representable using the
...
Algorithm: Kernel Perceptron Let![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.006.png) S be the empty set.

For all t = 1,2,...

1. Get next example (xt,yt)
1. Compute y t = sgn ys K(xs,xt) s∈S
1. If y t = yt add t to S

Therefore, ![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.007.png)
...
In other words, the feature map ϕ : Rd → RN associated with the polynomial kernel Kn sends

n

each x ∈ Rd to a vector whose components are indexed by all monomials k x

s=1 vs of degree at most n and weighted by the roots of the binomial coefficients.
...
In order to derive the map ϕγ associated with Kγ we proceeed as follows,

1 x − x′ 2 = exp − 1 ∥x∥2 + x′ 2 exp 1 ⊤ ′
...
- exp − exp − (5) 2γ![](Aspose.Words.81476c7a-1f55-4fbe-aea1-653552ec7103.008.png) 2γ n! γn

n=0

where we used the Taylor series expansion ex = 1 + x + x2 + ···. A closer look at (5) reveals that

2!

the Gaussian kernel is a linear combination of infinitely many polynomial kernels (3) of increasing degree, each weighted by the reciprocal of the factorial of its degree. The parameter γ is a scaling

factor for the products x⊤x′. Finally, the factors e∥x∥2/(2γ)e∥x′∥2/(2γ) normalize with respect to

x and x′ giving Kγ(x,x) = 1 for each x ∈Rd.

Note that predictors of the form (2) that utilize Gaussian kernels predict any point x using a linear

2/(2γ)

combination (with coefficients yt) of Gaussians e−z centered on xs for s ∈S and evaluated at x.

The Gaussian kernel is universal in the following sense: for each γ > 0, for each continuous function f : Rd → R, and for each ε > 0, there exists a function g of the form
...
for some x1,..., xN ∈ X, α1,...,αN ∈ R, and N ∈ N such that g approximates f with error bounded by ε. An important consequence of this fact is that learning algorithms that use Gaussian kernels can be consistent (that is, the expected risk of their predictors converges to the Bayes risk

as the sample size grows to infinity).

Given a data space X (not necessarily Rd) and a symmetric function K : X × X → R how can we

cϕhecK :kXwhether→ HK ,Kwhereis a Hkernel?K is a linearIn otherspacweords,withwinnere wanprot toductkno⟨·w,·⟩whetherK , suchtherethat existsϕK(x)a,ϕfeatureK (x′)map K = K(x,x′) for all x,x′ ∈ X. The space HK must be linear because our algorithms build linear

predictors using additive updates. Luckily, there is a very simple characterization: K is a kernel if and only if for all m ∈N and for all x1,..., xm ∈ X, the m×m matrix K such that K i,j = K(xi,xj) is positive semidefinite. That is, z⊤Kz ≥ 0 for all z ∈Rm.

The above result tells us the conditions under which ϕK exists for a given K. On the other hand, it does not tell us how ϕK looks like. In fact, there is no unique representation of the pair ϕK ,HK for a given kernel K. However, it can be shown that if K is a kernel, then we can always represent ϕK : X → HK as ϕK (x) = K(x,·). Now, since HK must be a linear space, we define it as the set of all linear combinations of K(x,·) for arbitrary choices of the coefficients and of the points x ∈ X,

N

HK ≡ αi K(xi,·) : x1,..., xN ∈ X, α1,...,αN ∈R, N ∈N

i=1

An element f ∈ HK is then a function f : X → R such that
...
for some x1,..., xN ∈ X, α1,...,αN ∈R, and N ∈N.
...
[^1]: The polynomial kernel Kn(x,x′) = 1 + x⊤x′ n for all n ∈ N generalizes the quadratic kernel defined earlier. Using Newton’s Binomial Theorem, we can explicitely compute the map ϕ such

    that Kn(x,x′) = ϕ (x)⊤ϕ (x′), n
...
- Write the upper bound on the estimation error of ERM run on a the class of complete binary tree predictors with at most N nodes on d binary features. Bonus points if you justify your answer with a proof.
