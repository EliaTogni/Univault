...
16) **Write the formula for Bayes optimal predictor and Bayes risk for the square loss.**
	The Bayes optimal predictor for the square loss is $f^*(x) = \mathbb{E}\big[Y \text{ } \vert \text{ } X = x\big]$ and the Bayes risk for the square loss is $\ell_{\mathcal{D}}(f^*) =  Var\big[Y \vert X \big]$.<br />
